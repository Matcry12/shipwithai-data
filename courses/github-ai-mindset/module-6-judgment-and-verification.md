...
And it matters because of a number you already met: AI-generated code "contains vulnerabilities 40% of the time, with accuracy rates of 65% for ChatGPT and 46% for Copilot on complex programming tasks" *(ksred.com)*. Put bluntly: a meaningful chunk of what the machine hands you is wrong or unsafe, every day, and it hands it to you **with total confidence**. The only thing standing between that and production is you, verifying. That's why this is the starred module. *(This ~40% is a second, independent source landing on the same figure as the "dangerous 40%" you met in Module 1 — there it was an academic study of nearly 1,700 Copilot-generated programs; here it's a separate industry analysis. Two different roads, same warning.)*
...
This isn't hypothetical. The same source points to real 2025 disasters: a dating app "leaking the private information of over 70,000 customers," and an AI agent "deleting [a company's] production database" *(coderabbit.ai)*. Confident code, real consequences.
...
- **ksred.com** — "The Vibe Coding Paradox" — AI code contains vulnerabilities ~40% of the time; accuracy ~65% (ChatGPT) / ~46% (Copilot) on complex tasks; juniors lack the pattern recognition to catch plausible-but-dangerous code.
...
- **coderabbit.ai** — "Vibe coding: Because who doesn't love surprise technical debt!?" — what AI doesn't do by default (tests, threat model, OWASP, input validation, responsible logging); real 2025 incidents (dating-app PII leak of 70k+, AI deleting a production database); struggle builds the instinct to "smell code rot before it crashes."
