...
source_domain: support.claude.com
topic: claude-code-workflow
doc_type: reference
published_date: '2026-04-15'
fetched_at: '2026-05-25T06:42:33.883889+00:00'
language: en
word_count: 1373
reading_time: 7
signal_score: 0.6782
status: kept
core_question: What are the primary ways developers use Claude Code across the software development lifecycle?
tldr: 'Documentation of ten common developer use cases for Claude Code: fixing failing tests, understanding
  code, finding features, triaging errors, refactoring, writing tests, reviewing PRs, onboarding, planning
  projects, and implementing features.'
key_topics:
- Claude Code
- debugging
- refactoring
...
entities:
  primary: Claude Code developer use cases
  aliases:
  - Claude Code applications
  - AI-assisted development tasks
  - development lifecycle
content_hash: sha256:4d068459bf54298fd938384ad622db6a398de6bfa55921caea347671e4565c83
---
...
**Prerequisite:** GitHub CLI authenticated (run `gh auth login`
...
scans the project and writes a `CLAUDE.md`

file summarizing build commands, architecture, and conventions. The follow-up tour prompt produces a guided walkthrough of the project structure. Project context remains loaded for the rest of the session.

**Reference:** **Memory and CLAUDE.md**
...
**Prerequisite:** Issue tracker connected via MCP (one-time `.mcp.json`
...
**Expected result:** Claude Code writes a `SKILL.md`

file to `.claude/skills/ship/`

. The `/ship`

command is available immediately, with no restart required, and is shared with anyone who clones the repository. (The legacy `.claude/commands/`

path is still supported, but skills are the recommended form.)
...
2 | Understand unfamiliar code |
3 | Find where something happens |
4 | Triage an error | Paste the stack trace, then ask: |
...
, @-references, permission modes, skills, MCP, and hooks, see **code.claude.com/docs**. Claude Code ships frequently—verify version-specific details against **code.claude.com/docs** before distributing internally.
