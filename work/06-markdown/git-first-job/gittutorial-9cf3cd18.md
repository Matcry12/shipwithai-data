---
title: Git
source_url: https://git-scm.com/docs/gittutorial
source_domain: git-scm.com
topic: git-first-job
doc_type: how-to-guide
published_date: '2026-04-20'
fetched_at: '2026-05-25T06:38:55.929569+00:00'
language: en
word_count: 3015
reading_time: 16
signal_score: 1.0
status: kept
core_question: What are the fundamental Git operations for managing code versions and collaboration?
tldr: 'Official Git tutorial covering basic operations: importing projects, making changes, committing,
  branching, merging, and viewing project history.'
key_topics:
- Git basics
- commits
- branching
...
- configuration
entities:
  primary: Git tutorial
  aliases:
  - Git basics
  - version control fundamentals
content_hash: sha256:d8177d2ee82675dc163e2a34b3a48011c6fa789ddb10e6cf964730f450ff706d
---
...
Assume you have a tarball `project.tar.gz`
...
summary of the situation with `git`
...
Alternatively, instead of running `git`
...
Many revision control systems provide an `add`

command that tells the
system to start tracking changes to a new file. Git's `add`

command
does something simpler and more powerful: `git`
...
to switch to the `experimental`

branch. Now edit a file, commit the
change, and switch back to the `master`
...
branch and you're back on the `master`

branch.

You can make a different change on the `master`
...
made in each. To merge the changes made in `experimental`
...
At this point you could delete the `experimental`
...
If you develop on a branch `crazy-idea`
...
This creates a new directory `myrepo`
...
This merges the changes from Bob's `master`
...
The range notation `HEAD..FETCH_HEAD`

means "show everything that is reachable
from the `FETCH_HEAD`

but exclude anything that is reachable from `HEAD`

".
Alice already knows everything that leads to her current state (`HEAD`

),
and reviews what Bob has in his state (`FETCH_HEAD`
...
stash her work-in-progress first, do a `pull`
...
remote repository shorthand set up with `git`
...
could merge the changes into her `master`
...
can also be done by *pulling from her own remote-tracking
branch*, like this:
...
(The complete configuration created by `git`
...
, and the git-config[1] man page
explains the meaning of each option.)

Git also keeps a pristine copy of Alice's `master`
...
have already seen that the `git`

`log`

command can list those commits.
Note that first line of each `git`
...
commits, they will be lost. Also, don't use `git`
...
If you need to undo changes that you have pushed, use `git`
...
If you leave out the commit name, `git`
...
in a number of ways. Here are some examples with `git`
...
will list commits made in the `master`
...
then merged back together, the order in which `git`
...
or Git itself) have frequent merges, and `gitk`
...
that modified files under the `drivers`
...
git-bisect[1]: When there is a regression in your project, one way to track down the bug is by searching through the history to find the exact commit that's to blame.
