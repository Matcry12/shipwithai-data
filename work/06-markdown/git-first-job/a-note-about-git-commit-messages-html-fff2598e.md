---
title: tbaggery
source_url: https://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html
source_domain: tbaggery.com
topic: git-first-job
doc_type: how-to-guide
author: Tpope
published_date: '2008-04-19'
fetched_at: '2026-05-25T06:42:39.357361+00:00'
language: en
word_count: 720
reading_time: 4
signal_score: 1.0
status: kept
core_question: What constitutes a well-formatted Git commit message?
tldr: 'Best practices for writing well-formatted Git commit messages: 50-character subject line, 72-column
  body wrapping, imperative voice, and blank line separation for tools integration.'
key_topics:
- Git commit messages
...
- email integration
entities:
  primary: Git commit message best practices
  aliases:
  - commit message formatting
  - Git message conventions
  - commit message standards
content_hash: sha256:b8a1adba972ff796d854cd613e7dd83cae243af019f95b47a3f08946765bfcf6
---
...
, this means your paragraphs flow far off the edge of the screen, making them difficult to read. On an 80 column terminal, if we subtract 4 columns for the indent on the left and 4 more for symmetry on the right, we're left with 72 columns.`git format-patch --stdout`
...
For Textmate, you can adjust the "Wrap Column" option under the view
menu, then use `^Q`

to rewrap paragraphs (be sure there's a blank line
afterwards to avoid mixing in the comments). Here's a shell command to
add 72 to the menu so you don't have to drag to select each time:
...
shows a terse history mapping containing the commit id and the summary`git rebase --interactive`

provides the summary for each commit in the editor it invokes- if the config option
`merge.summary`

is set, the summaries from all merged commits will make their way into the merge commit message `git shortlog`

uses summary lines in the changelog-like output it produces`git format-patch`

,`git send-email`

, and related tools use it as the subject for emails- reflogs, a local history accessible with
`git reflog`

intended to help you recover from stupid mistakes, get a copy of the summary `gitk`

has a column for the summary- GitHub uses the summary in various places in their user interface
