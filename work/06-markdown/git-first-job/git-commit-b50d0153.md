...
source_url: https://cbea.ms/git-commit/
source_domain: cbea.ms
topic: git-first-job
doc_type: how-to-guide
author: Cbeams
published_date: '2014-08-30'
fetched_at: '2026-05-25T06:37:35.899935+00:00'
language: en
word_count: 2825
reading_time: 15
signal_score: 0.9941
status: kept
core_question: What are the rules for writing effective Git commit messages?
tldr: Guide on writing effective Git commit messages following seven rules including subject-body separation,
  imperative mood, and clear explanations of what and why.
key_topics:
...
entities:
  ...
  - Git message style
content_hash: sha256:cb9d32dabc8e862026fe672f2d461bc59c5d80c6b6b64616843c0b0635634f13
---
...
If you haven't given much thought to what makes a great Git commit message, it may be the case that you haven't spent much time using `git log`

and related tools. There is a vicious cycle here: because the commit history is unstructured and inconsistent, one doesn't spend much time using or taking care of it. And because it doesn't get used or taken care of, it remains unstructured and inconsistent.

But a well-cared for log is a beautiful and useful thing. `git blame`
...
Nothing more need be said; if the reader wonders what the typo was, she can simply take a look at the change itself, i.e. use `git show`

or `git diff`

or `git log -p`

If you're committing something like this at the command line, it's easy to use the `-m`
...
option. You're better off writing the message in a proper text editor. If you do not already have an editor set up for use with Git at the command line, read this section of Pro Git.
...
And now `git log --oneline`

, which prints out just the subject line:
...
, which groups commits by user, again showing just the subject line for concision:
...
__your subject line here__

For example:

- If applied, this commit will
*refactor subsystem X for readability* - If applied, this commit will
*update getting started documentation* - If applied, this commit will
*remove deprecated methods* - If applied, this commit will
*release version 1.0.0* - If applied, this commit will
...
Certain Git-related IDE functions are invaluable, like calling `git rm`

when you delete a file, and doing the right stuff with `git`

when you rename one. Where everything falls apart is when you start trying to commit, merge, rebase, or do sophisticated history analysis through the IDE.
