...
fetched_at: '2026-05-29T13:38:02.806471+00:00'
language: en
word_count: 3262
reading_time: 17
signal_score: 1.0
status: kept
core_question: What is spec-driven development and how do different tools implement it?
tldr: Analysis of three spec-driven development tools (Kiro, Spec-kit, Tessl) exploring the definition,
  implementation levels, and practical challenges of using specifications to guide AI-assisted development.
key_topics:
...
entities:
  primary: spec-driven development tools
  aliases:
  - SDD tools
  - specification-first development
content_hash: sha256:7ef6cf1023a7ecfead1c859b5a8ddb768bdfa5ed5a72722e7f28f9a6736500b5
---
...
Tessl is the only one of these three tools that explicitly aspires to a spec-anchored approach, and is even exploring the spec-as-source level of SDD. A Tessl spec can serve as the main artifact that is being maintained and edited, with the code even marked with a comment at the top saying `// GENERATED FROM SPEC - DO NOT EDIT`

. This is currently a 1:1 mapping between spec and code files, i.e. one spec translates into one file in the codebase. But Tessl is still in beta and they are experimenting with different versions of this, so I can imagine that this approach could also be taken on a level where one spec maps to a code component with multiple files. It remains to be seen what the alpha product will support. (The Tessl team themselves see their framework as something that is more in the future than their current public product, the Tessl Registry.)

Here is an example of a spec that I had the Tessl CLI reverse engineer (`tessl document --code ...js`

) from a JavaScript file in an existing codebase:

Tags like `@generate`

or `@test`

seem to tell Tessl what to generate. The API section shows the idea of defining at least the interfaces that get exposed to other parts of the codebase in the spec, presumably to make sure that these more crucial parts of the generated component are fully under the control of the maintainer. Running `tessl build`
