---
title: "If tests fail, prevent the commitif[ $?-ne 0];then"
topic: "git-first-job"
career_level:
  - executive
...
![](https://algocademy.com/blog/wp-content/uploads/2024/10/compressed_image-527-1024x585.webp)
...
### Key Git Concepts
  ...
  * **Branch:** A parallel version of the repository that allows you to work on different features independently.
...
GitHub Flow is a simpler alternative to GitFlow, suitable for teams that deploy frequently:
  * The **main** branch always contains deployable code.
...
subject of an email and the rest of the text as the body.-Bullet points are okay, too
...
```
#!/bin/sh# Pre-commit hook to run tests
npm test

# If tests fail, prevent the commitif[ $?-ne 0];then
  echo "Tests failed. Commit aborted."exit1fi
```
