...
source_domain: "www.producttalk.org"
word_count: 4373
text_to_link_ratio: 0.7975
signal_score: 0.8975
is_curated: false
tags:
  - open-source
...
I've been going deep on [Claude Code](https://claude.ai/code/onboarding?ref=producttalk.org). I'm finding this has been a better strategy for getting value out of AI—rather than trying every new tool on the market.
...
🎖️ This Product Talk Article is brought to you by [New Year, New Habit: The 5-Day Customer Interview Challenge](https://learn.producttalk.org/5-day-customer-interview-challenge-january2026?utm_source=Product+Talk&utm_medium=how-to-use-claude-code-features-ad1). Become a more confident interviewer in less than a week. You'll conduct one practice interview a day, get personalized and detailed feedback so you know exactly what to improve, and we'll be giving out daily prizes to the most improved. [Join the challenge today.](https://learn.producttalk.org/5-day-customer-interview-challenge-january2026?utm_source=Product+Talk&utm_medium=how-to-use-claude-code-features-ad1)
🎖️ This Product Talk Article is brought to you by [Just Now Possible](https://producttalk.org/just-now-possible?utm_source=Product+Talk&utm_medium=how-to-use-claude-code-features-ad2), a podcast about how AI products come to life—straight from the builders. If you are being asked to add AI features to your roadmap, you don't have to start from scratch. Get a head start by hearing how other teams are navigating similar challenges. Find it on [YouTube,](https://www.youtube.com/playlist?list=PLD4atqlPcvA7m-D5noA_Pig6zk7n-LFn2&ref=producttalk.org) [Apple Podcasts](https://podcasts.apple.com/us/podcast/just-now-possible/id1838832993?ref=producttalk.org), and [Spotify](https://open.spotify.com/show/2ktQZOfW0AUF6bAFKxVXe1?ref=producttalk.org).
...
![The three-layered context system: 1. Global preferences in ~/.claude/CLAUDE.md, 2. Project context in project/CLAUDE.md, and custom reference files to be used as needed.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/3-layered-claude-code-memory-strategy-1.webp)Give Claude the right context for the task at hand.
I've already covered using markdown files to manage Claude's context in this article: [Stop Repeating Yourself: Give Claude Code a Memory](https://www.producttalk.org/give-claude-code-a-memory/).
...
If you want to learn more about this strategy, read: [Stop Repeating Yourself: Give Claude Code a Memory](https://www.producttalk.org/give-claude-code-a-memory/)
...
When I first started using [ChatGPT](https://chatgpt.com/?ref=producttalk.org) and Claude in the browser, I started saving my commonly used prompts in [Apple Notes](https://www.icloud.com/notes?ref=producttalk.org). Instead of having to start from scratch each time, I'd simply copy and paste.
...
![The competitive research slash command has a five-step workflow including reading files, launching parallel agents to do research, waiting for completion, and finally generating pricing and feature tables.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/Competitive-Research-Slash-Commands-1.webp)An overview of the competitive research slash command that we previously created.
Thankfully, Claude Code has slash commands—named as such because they start with a /. In [the first article in this series](https://www.producttalk.org/claude-code-what-it-is-and-how-its-different/), we created a /competitive-research command and I've also shared details about my /today command and many of my writing slash commands (e.g. /headlines, /seo).
...
![My headlines slash command has a clearly define process. The agent reads the articles, checks to see if an SEO analysis already exists, generates 3-5 headlines in each of 7 categories. It writes them to a file and then provides a summary.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/Headlines-Slash-Command-2.webp)
![My seo slash command follows a more complex process. It does an in-depth content analysis and keyword research. And then it uses an API to look up search volume and analyze trends.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/SEO-Slash-command-1.webp)
...
In Claude Code, there are three types of agents.
  1. Claude is itself an agent—meaning it has access to a set of tools, decides on its own when and how to use those tools, and makes a decision about when it has completed its task.
  2. Claude can spawn sub-agents. If you see Claude use the Task command, that's Claude spinning up a general-purpose agent to help it do something.
  3. You can define user-designed sub-agents and tell Claude exactly when and how to use them.
...
Claude Code (in [VS Code](https://code.visualstudio.com/?ref=producttalk.org)) recently started compacting the context window when it gets to 75% full. This is a way of ensuring higher-quality output.
...
![A screenshot of the technical-blog-reviewer sub-agent running in the background with the main Claude agent waiting for results.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/WebSearch-as-a-Sub-Agent.webp)My technical-blog-reviewer sub-agent is reviewing my blog post in its own context window and will report back to the main agent when it's done.
...
![A screenshot of Claude launching 15 task agents, each working with their own context window.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/Claude-Code-Agents-working-in-parallel-1.webp)Claude launches 15 Task agents to research 15 different competitors. Each agent works with its own context window.
...
Front matter is a small block of information at the very top of a file that describes the file itself—like a label on a folder. It sits between two lines of three dashes (`---`) and contains simple details like a description or title. You don't need to memorize the format—just ask Claude to create the slash command for you and it will handle the front matter automatically.
...
![A screenshot of a file system with ab-test, architecture, process-notes, and readme folders. The process-notes and readme folders are expanded to show that they each include a SKILL.md file.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/Where-Skills-Live.webp)Skills are always saved in a SKILL.md file. The folder they are saved in determines the name of the skill. In this project, I have an ab-test skill, an architecture skill, a process-notes skill, and a readme skill. 
...
![A screenshot showing the hooks interface. The user can select a hook by typing in a number 1-5.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/Hooks-Interface.webp)The Claude Code interactive hooks interface.
...
![A screenshot of a public repository on GitHub for the official Claude Code plug-ins.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/claude-plugins-official-on-github.webp)Anthropic's Claude Code Plug-ins marketplace.
Technically, both plug-ins and marketplaces are just public git repositories hosted on [GitHub](https://github.com/?ref=producttalk.org). If you are new to git and GitHub, here's what you need to know: Git is software that helps software engineers manage code versions and conflicts when multiple engineers work on the same code base. A repository is just a collection of related code. GitHub is a cloud host of git repositories.
...
![A screenshot of the "Add Marketplace" flow in Claude Code.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/claude-s-marketplace.webp)You can add a marketplace directly in Claude Code by using /plugin.
...
![A screenshot of the Discover screen showing several available plug-ins from Anthropic.](https://storage.ghost.io/c/57/9b/579b6dca-f48a-4307-844f-f0533595d058/content/images/2026/04/anthropic-plugins.webp)Once you install a marketplace, you can use /plugin to access the Discover tab and explore the plug-ins that are part of that marketplace.
Then from within the /plugin interface you can tab to Discover to see what plug-ins they've made available.
And if you are familiar with GitHub, you can [explore their public repos here.](https://github.com/anthropics/claude-plugins-official?ref=producttalk.org)
This is a great way to use workflows that other people have created. However, remember, plug-ins can include code (both in skills and in hooks). You should never install and let Claude run code on your computer that you aren't familiar with if you haven't reviewed it first. Revisit [our safety article](https://www.producttalk.org/how-to-use-claude-code-safely/) if you need a refresher on how you can do this.
...
#### Read next
[ Audio Version ($) I've been hosting Claude Code office hours for the past few months and by far the most common questions I get are around sharing context and skills. I'm hearing from a lot of people who are leading AI adoption in their companies. They' ](https://www.producttalk.org/how-to-share-your-ai-context-and-skills/) [ Audio Version ($) We need to talk about product roadmaps. I once worked at a startup where our product roadmap was literally an Excel spreadsheet full of feature ideas. Most of them came from our CEO. Our head of product simply prioritized the rows. Based on what, I have no idea. ](https://www.producttalk.org/product-roadmaps/) [ Audio Version ($) - This link will only work for paid subscribers. I've written more software in the past six months than I've written in my entire lifetime. I started vibe coding in March 2025. My first foray was extraordinarily successful and then resulted in complete failure. ](https://www.producttalk.org/vibe-coding-best-practices/)
## Make better product decisions.
Get actionable insights delivered straight to your inbox.
