# Keyword & Topic Frequency Analysis

**Generated:** 2026-05-26  
**Source:** V2 enrichment (key_topics + entities) over 589 kept articles

Top topic tags and primary entities per topic, aggregated from the LLM
enrichment layer (not raw text frequency).

---

## ats-optimization

_54 articles · doc types: how-to-guide 33, reference 17, listicle 2, news 1, opinion 1_

**Top key_topics:**

| Keyword | Count |
|---|---|
| ats optimization | 14 |
| ats keywords | 10 |
| technical skills | 7 |
| resume optimization | 6 |
| keywords | 6 |
| ats systems | 5 |
| resume formatting | 5 |
| keyword optimization | 5 |
| resume templates | 5 |
| hard skills | 5 |
| soft skills | 5 |
| keyword matching | 4 |
| keyword placement | 4 |
| job descriptions | 4 |
| job search | 3 |
| section headers | 3 |
| strategic vision | 3 |
| resume keywords | 3 |
| resume bullets | 3 |
| formatting | 3 |
| action verbs | 2 |
| applicant tracking | 2 |
| file format | 2 |
| ats compatibility | 2 |
| formatting rules | 2 |

**Top primary entities:**

- ats resume optimization (4)
- ats resume templates (3)
- ats resume keywords (3)
- ats-friendly resume templates (2)
- ats and ai resume optimization (1)
- applicant tracking system resume anxiety (1)
- ats-friendly resume creation (1)
- ats resume for it engineers (1)
- ats resume screening (1)
- software engineer ats resume (1)

---

## career-change

_47 articles · doc types: how-to-guide 35, reference 7, case-study 5_

**Top key_topics:**

| Keyword | Count |
|---|---|
| transferable skills | 19 |
| career change | 12 |
| ats optimization | 7 |
| networking | 7 |
| career transition | 6 |
| certifications | 5 |
| resume format | 4 |
| technical skills | 4 |
| job search | 4 |
| adaptability | 3 |
| portfolio building | 3 |
| leadership | 3 |
| work experience | 3 |
| education | 3 |
| soft skills | 3 |
| skill identification | 3 |
| problem-solving | 3 |
| resume objective | 2 |
| resume examples | 2 |
| cv template | 2 |
| professional summary | 2 |
| skill mapping | 2 |
| resume writing | 2 |
| software engineer resume | 2 |
| entry-level engineer | 2 |

**Top primary entities:**

- career change resume guide (3)
- career change resume writing (2)
- career change to software engineering (2)
- bootcamp graduate job search (2)
- career transition into tech (2)
- transferable skills (2)
- career change resume examples (1)
- career change cv template (1)
- career change resume transitions (1)
- functional career change resumes (1)

---

## career-gap

_49 articles · doc types: how-to-guide 36, reference 8, opinion 3, case-study 2_

**Top key_topics:**

| Keyword | Count |
|---|---|
| career break | 10 |
| career breaks | 9 |
| employment gaps | 7 |
| employment gap | 6 |
| transferable skills | 6 |
| career gaps | 5 |
| job search | 4 |
| resume formatting | 4 |
| skill development | 4 |
| resume gaps | 4 |
| resume writing | 4 |
| interview preparation | 4 |
| maternity leave | 4 |
| cover letter | 3 |
| caregiving | 3 |
| parental leave | 3 |
| confidence building | 3 |
| gap explanation | 3 |
| functional resume | 2 |
| professional development | 2 |
| cv format | 2 |
| employment history | 2 |
| honesty | 2 |
| resume examples | 2 |
| sabbatical | 2 |

**Top primary entities:**

- career break explanation (2)
- employment gap explanation (2)
- employment gap career advice (1)
- mid-level developer job search (1)
- post-graduation employment gap (1)
- resume updates after maternity leave (1)
- software engineer resume mistakes (1)
- career break cv template (1)
- career break on resume (1)
- career break resume samples (1)

---

## claude-code-workflow

_40 articles · doc types: how-to-guide 22, reference 12, opinion 4, case-study 2_

**Top key_topics:**

| Keyword | Count |
|---|---|
| claude code | 7 |
| code review | 4 |
| refactoring | 4 |
| ai coding assistants | 4 |
| github copilot | 4 |
| pair programming | 4 |
| code generation | 4 |
| context window | 4 |
| debugging | 3 |
| prompt engineering | 3 |
| slash commands | 3 |
| plan mode | 3 |
| claude.md | 3 |
| installation | 3 |
| development workflows | 2 |
| ai productivity | 2 |
| coding assistants | 2 |
| tool selection | 2 |
| ai impact | 2 |
| code quality | 2 |
| context management | 2 |
| workflow automation | 2 |
| multi-file editing | 2 |
| codebase navigation | 2 |
| github integration | 2 |

**Top primary entities:**

- ai coding assistant best practices (2)
- claude code workflow optimization and skills (1)
- claude code developer use cases (1)
- claude code reviews (1)
- ai coding assistant productivity (1)
- selecting ai coding assistants (1)
- ai-powered developer productivity (1)
- ai assistance impact on coding skill development (1)
- ai-assisted coding best practices (1)
- ai developer productivity workflow (1)

---

## cover-letter

_21 articles · doc types: how-to-guide 17, reference 3, opinion 1_

**Top key_topics:**

| Keyword | Count |
|---|---|
| cover letter template | 4 |
| cover letters | 4 |
| internal applications | 3 |
| career development | 3 |
| transferable skills | 3 |
| cover letter structure | 3 |
| achievements | 2 |
| professional tone | 2 |
| customization | 2 |
| technical skills | 2 |
| networking | 2 |
| hiring manager | 2 |
| resume | 2 |
| career growth | 1 |
| company knowledge | 1 |
| track record | 1 |
| professional relationships | 1 |
| accomplishments | 1 |
| company goals | 1 |
| measurable results | 1 |
| office politics | 1 |
| company culture | 1 |
| professional narrative | 1 |
| software engineer | 1 |
| achievement highlight | 1 |

**Top primary entities:**

- internal position cover letter (3)
- software engineer cover letter (2)
- software engineer cover letter template (1)
- cover letter not fully qualified (1)
- cover letter templates (1)
- cover letter templates canva (1)
- developer cover letter (1)
- front-end developer cover letter (1)
- developer cover letters (1)
- internal job transfer strategy (1)

---

## executive-resume

_49 articles · doc types: how-to-guide 24, reference 23, listicle 1, opinion 1_

**Top key_topics:**

| Keyword | Count |
|---|---|
| team management | 9 |
| technical skills | 7 |
| cto resume | 5 |
| technical leadership | 5 |
| team leadership | 5 |
| strategic planning | 4 |
| career progression | 4 |
| resume format | 4 |
| leadership skills | 4 |
| leadership | 3 |
| technical expertise | 3 |
| quantifiable achievements | 3 |
| work experience | 3 |
| soft skills | 3 |
| resume writing | 3 |
| engineering leadership | 2 |
| team building | 2 |
| certifications | 2 |
| resume formatting | 2 |
| executive leadership | 2 |
| technical vision | 2 |
| achievements | 2 |
| resume design | 2 |
| professional summary | 2 |
| resume templates | 2 |

**Top primary entities:**

- director of engineering resume (2)
- cto resume examples (2)
- vp engineering chief architect resume (1)
- senior director of digital platform engineering resume (1)
- cto cover letter example (1)
- continuing education on resume (1)
- cto resume (1)
- cto resume vietnamese (1)
- cto resume guide (1)
- cto resume writing (1)

---

## git-first-job

_54 articles · doc types: how-to-guide 42, reference 12_

**Top key_topics:**

| Keyword | Count |
|---|---|
| version control | 12 |
| merge conflicts | 9 |
| pull requests | 6 |
| branching | 6 |
| feature branches | 5 |
| github | 5 |
| gitflow | 5 |
| commit messages | 4 |
| github flow | 4 |
| merging | 4 |
| conflict markers | 4 |
| code review | 4 |
| git reset | 3 |
| git workflow | 3 |
| git basics | 3 |
| repository setup | 3 |
| trunk-based development | 3 |
| conflict resolution | 3 |
| git commits | 2 |
| claude code | 2 |
| git checkout | 2 |
| git rebase | 2 |
| release management | 2 |
| team workflow | 2 |
| rebase | 2 |

**Top primary entities:**

- code review best practices (3)
- git commit message best practices (2)
- git branching strategies (2)
- git merge conflicts (2)
- git tutorial (2)
- git merge conflict resolution (2)
- commit message generation prompt (1)
- common git problems and fixes (1)
- git-flow branching model (1)
- advanced git commands and workflows (1)

---

## github-portfolio

_62 articles · doc types: how-to-guide 30, reference 23, opinion 5, listicle 2, news 1, case-study 1_

**Top key_topics:**

| Keyword | Count |
|---|---|
| open source | 12 |
| portfolio | 9 |
| github profile | 8 |
| github portfolio | 7 |
| project documentation | 7 |
| github | 6 |
| code quality | 6 |
| project showcase | 5 |
| ci/cd | 4 |
| react | 4 |
| ats optimization | 4 |
| readme documentation | 3 |
| portfolio projects | 3 |
| cover letters | 3 |
| readme files | 3 |
| pinned repositories | 3 |
| profile readme | 3 |
| portfolio strategy | 3 |
| microservices | 3 |
| kubernetes | 3 |
| hiring | 3 |
| resume building | 3 |
| career development | 2 |
| portfolio building | 2 |
| developer portfolio | 2 |

**Top primary entities:**

- github profile readme (2)
- github profile optimization (2)
- github profile for job search (2)
- github profile (2)
- open-source projects for developer portfolio (1)
- github as job application portfolio (1)
- beginner python projects (1)
- professional github profile (1)
- hiring github developers (1)
- github resume feature proposal (1)

---

## linkedin-profile

_29 articles · doc types: how-to-guide 22, reference 5, listicle 2_

**Top key_topics:**

| Keyword | Count |
|---|---|
| linkedin headline | 7 |
| keywords | 6 |
| linkedin profile | 4 |
| profile optimization | 4 |
| specialization | 3 |
| technical skills | 3 |
| about section | 3 |
| professional narrative | 3 |
| software engineers | 3 |
| keyword optimization | 3 |
| linkedin | 3 |
| specializations | 3 |
| computer vision | 2 |
| deep learning | 2 |
| industry focus | 2 |
| ai applications | 2 |
| career paths | 2 |
| recruitment | 2 |
| career progression | 2 |
| linkedin summary | 2 |
| call to action | 2 |
| headline optimization | 2 |
| certifications | 2 |
| content strategy | 2 |
| networking | 2 |

**Top primary entities:**

- linkedin headline for computer vision engineers (2)
- linkedin summary writing (2)
- linkedin profile optimization for developers (1)
- linkedin profile optimization for junior developers (1)
- resume action verbs (1)
- ats resume writing (1)
- linkedin for developers (1)
- design engineer career (1)
- engineering linkedin headlines (1)
- engineering manager career (1)

---

## remote-work-resume

_62 articles · doc types: how-to-guide 48, reference 12, other 1, listicle 1_

**Top key_topics:**

| Keyword | Count |
|---|---|
| remote work | 9 |
| remote tools | 9 |
| distributed teams | 8 |
| ats optimization | 8 |
| soft skills | 8 |
| technical skills | 7 |
| async communication | 6 |
| job search | 5 |
| remote work skills | 5 |
| ats keywords | 4 |
| resume template | 4 |
| work-from-home | 4 |
| resume writing | 4 |
| quantified results | 3 |
| remote resume | 3 |
| communication skills | 3 |
| teamwork skills | 3 |
| collaboration | 3 |
| resume optimization | 2 |
| resume format | 2 |
| documentation | 2 |
| resume formatting | 2 |
| digital tools | 2 |
| time management | 2 |
| formatting | 2 |

**Top primary entities:**

- remote work resume (2)
- remote job resume (2)
- teamwork skills on resume (2)
- beginner web developer resume (1)
- senior backend engineer job (1)
- remote-ready resume optimization (1)
- mid-level developer resume (1)
- android developer resume guide (1)
- asynchronous video interviews (1)
- asynchronous work (1)

---

## salary-negotiation

_52 articles · doc types: how-to-guide 34, reference 9, opinion 7, news 2_

**Top key_topics:**

| Keyword | Count |
|---|---|
| salary negotiation | 26 |
| compensation | 12 |
| equity | 8 |
| job offers | 7 |
| total compensation | 5 |
| market research | 5 |
| negotiation tactics | 5 |
| salary research | 4 |
| benefits negotiation | 4 |
| return offer | 4 |
| job offer | 4 |
| leverage | 4 |
| benefits | 4 |
| stock options | 3 |
| negotiation strategy | 3 |
| entry-level negotiation | 3 |
| offer negotiation | 3 |
| negotiation | 3 |
| market rates | 2 |
| batna | 2 |
| negotiation email | 2 |
| market data | 2 |
| communication | 2 |
| networking | 2 |
| equity compensation | 2 |

**Top primary entities:**

- entry-level salary negotiation (2)
- return offer salary negotiation (2)
- developer salary negotiation (2)
- salary negotiation (2)
- salary negotiation for engineers (1)
- salary negotiation email templates (1)
- salary negotiation for new graduates (1)
- amazon intern return offer negotiation (1)
- total compensation negotiation (1)
- counter offer email templates (1)

---

## senior-level-resume

_70 articles · doc types: how-to-guide 38, reference 25, opinion 3, case-study 2, listicle 1, other 1_

**Top key_topics:**

| Keyword | Count |
|---|---|
| technical skills | 13 |
| career progression | 10 |
| leadership | 9 |
| mentorship | 9 |
| resume writing | 8 |
| technical leadership | 7 |
| resume examples | 7 |
| principal engineer | 6 |
| staff engineer | 6 |
| metrics | 5 |
| ats optimization | 5 |
| technical expertise | 5 |
| team management | 4 |
| career growth | 4 |
| engineering roles | 4 |
| resume templates | 4 |
| senior engineer | 4 |
| career levels | 4 |
| project management | 4 |
| cloud platforms | 3 |
| resume template | 3 |
| team leadership | 3 |
| resume format | 3 |
| resume sections | 3 |
| individual contributor | 3 |

**Top primary entities:**

- senior engineering manager resume (4)
- junior to senior developer progression (2)
- principal engineer resume (2)
- staff engineer resume (2)
- staff engineer vs senior engineer (2)
- senior software engineer resume examples (2)
- software engineer resume (2)
- individual contributor resume example (1)
- resume action verbs (1)
- junior to senior developer growth (1)

---
